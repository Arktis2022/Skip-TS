...
def train(class_,epochs,learning_rate,res,batch_size,print_epoch,seg,data_path,save_path,print_canshu,score_num,print_loss,img_path,vis,cut,layerloss,rate,print_max,net,L2,seed): 
    ...
    optimizer = torch.optim.Adam(list(decoder.parameters())+list(encoder.parameters()), lr=learning_rate, betas=(0.5,0.999)) # 给它一个可进行迭代优化的包含了所有参数（所有的参数必须是变量s）的列表
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path',default='/data/liumingxuan/mvtec_anomaly_detection/',type=str) # 选择数据集文件夹位置
